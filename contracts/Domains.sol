...
contract Domains is ERC721URIStorage {
    ...
    mapping(string => string) public records; // allowing domains to set a string with their domains.
    ...
    constructor(string memory _tld) payable ERC721("As Fuck Name Service", "AF"){
    ...
    function register(string calldata name) public payable {
        ...
        if (!isDomainValid(name)) revert InvalidName(name);

        uint _price = price(name);
        console.log("Price for name %s is %u", name, _price);
        console.log("ms.value is " , msg.value);
        require(msg.value >= _price, "Not enough Matic paid");

        string memory domainName = string(abi.encodePacked(name, ".", tld));
        string memory finalSvg = string (abi.encodePacked(svgPartOne, domainName, svgPartTwo));
        ...
        string memory nftMetadataJson = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name":"',
                        domainName, 
                        '", "description" : "This is a domain name service on as fuck domain", "image": "data:image/svg+xml;base64,',
        ...
        string memory finalTokenUri = string( abi.encodePacked("data:application/json;base64,", nftMetadataJson));
        ...
        console.log("%s has registered a domain named %s", msg.sender, name);
    ...
    function isDomainValid(string calldata name) public pure returns(bool) {
